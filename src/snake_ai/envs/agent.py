...
class Agent(metaclass=ABCMeta):
    ...
    def position(self, rect : Rectangle):
        if rect.width != self.pixel or rect.height != self.pixel:
            raise ValueError("Can not assign a new position with different pixel size." + 
                             f"Current pixel size  : {self.pixel}. Rectangle pixel size : {rect.width, rect.height}")
        self._position = rect

    @property
    def grid_position(self) -> Tuple[int]:
        """Tuple representing the position (x,y) of the agent on a grid"""
        return self._position.x // self.pixel, self._position.y // self.pixel
    ...
    def direction(self, direction : Direction):
        ...
        if direction.name not in ['NORTH', 'EAST', 'SOUTH', 'WEST']:
            raise errors.UnknownDirection(f"The direction {direction} is unknown. Available directions are : 'NORTH', 'EAST', 'SOUTH', 'WEST'.") 
