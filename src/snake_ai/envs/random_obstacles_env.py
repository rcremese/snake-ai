...
from snake_ai.utils import Colors, Direction, Reward, errors
...
class RandomObstaclesEnv(GridWorld):
    ...
    def goal(self) -> Rectangle:
        ...
        if self._goal is None:
            raise errors.InitialisationError("The goal variable is not initialized. Reset the environment !")
        return self._goal
    ...
    def position(self) -> Rectangle:
        ...
        if self._position is None:
            raise errors.InitialisationError("The position variable is not initialized. Reset the environment !")
    ...
    def direction(self) -> Direction:
        ...
        if self._direction is None:
            raise errors.InitialisationError("The direction argument is not initialized. Reset the environment !")
    ...
    def obstacles(self) -> List[Rectangle]:
        ...
        if self._obstacles is None:
            raise errors.InitialisationError("The obstacles are not initialized. Reset the environment !")
    ...
    def observations(self) -> np.ndarray:
        ...
        """
        if self._position is None:
            raise errors.InitialisationError("The position is not initialised. Reset the environment first !")
    ...
    def free_positions(self) -> List[Tuple[int, int]]:
        ...
        if self._free_position_mask is None:
            raise errors.InitialisationError("The free position mask is not initialised. Reset the environment first !")
    ...
    def _sanity_check(self, rect : Rectangle, max_size : int):
        ...
        if (rect.x < 0) or (rect.x > self.window_size[0]) or (rect.y < 0) or (rect.y > self.window_size[1]):
            raise errors.OutOfBoundsError(f"The rectangle position ({rect.x}, {rect.y}) is out of bounds {self.window_size}")
        if any([corner % self.pixel != 0 for corner in [rect.x, rect.y, rect.width, rect.height]]):
            raise errors.ResolutionError(f"The rectangle positions and lengths need to be a factor of pixel size : {self.pixel}.")
        if rect.height > max_size or rect.height > max_size:
            raise errors.ShapeError(f"The rectangle length can not be greater than {max_size}. " +
    ...
    def _populate_grid_with_obstacles(self) -> List[Rectangle]:
        ...
        if total_area >= self.width * self.height - 2:
            raise errors.ConfigurationError(f"There are too much obstacles ({self.nb_obs}) or with a range which is too wide {self._max_obs_size} for the environment ({self.width},{self.height}).")
