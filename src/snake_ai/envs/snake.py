...
def get_direction_from_positions(head: Rectangle, neck : Rectangle) -> Direction:
    ...
    """
    assert (head.width == neck.width) and (head.height == neck.height), f"The 2 arguments head {head} and neck {neck} need to have the same width and height."
...
class BidirectionalSnake(Snake):
    ...
    def move_from_action(self, action : int):
        assert action in range(4), f"Action need to be an integer in the range [0,3]. Get {action}."
