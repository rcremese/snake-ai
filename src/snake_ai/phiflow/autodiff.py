...
def stochastic_walk_simulation(
    ...
    while i < int(nb_iter):
        ...
        update = dt * flow.field.sample(force_field, points) + flow.math.sqrt(
            2 * dt * diffusivity
        ) * flow.math.random_normal(points.shape)
    ...
    vis.animate_walk_history(
        history,
        log_concentration,
        output=simulation_path.joinpath("differentiated_walkers.gif"),
        target=target,
...
def trajectory_optimization(
    point_cloud: flow.PointCloud,
    initial_field: flow.CenteredGrid,
    target: flow.geom.Point,
    obstacles: List[flow.Geometry],
    max_epoch: int,
    lr: float,
    nb_iter: int,
    dt: float,
    ...
    assert (
        gradient_clip is None or gradient_clip > 0
    ), f"Expected gradient_clip to be > 0 or None, get {gradient_clip}"
    ...
    def simulation(
        ...
        trajectories = stochastic_walk_simulation(
            point_cloud,
            force_field,
            dt=dt,
            nb_iter=nb_iter,
    ...
    for epoch in range(max_epoch):
        ...
        if gradient_clip is not None:
            force_field = maths.clip_gradient_norm(
                force_field, threashold=gradient_clip
            )
...
if __name__ == "__main__":
    ...
    # simulation_path = dirpath.joinpath("RoomEscape(20,20)_pixel_Tmax=1600.0_D=1/seed_0")
