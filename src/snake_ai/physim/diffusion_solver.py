...
def explicit_diffusion(
    concentration: flow.field.Field, diffusion: Numerical, dt: Numerical
) -> flow.field.Field:
    return flow.diffuse.explicit(concentration, diffusivity=diffusion, dt=dt)


@flow.math.jit_compile
def explicit_diffusion_with_obstacles(
    ...
    return (1 - obstacle_mask) * flow.diffuse.explicit(
        concentration, diffusivity=diffusion, dt=dt
    )
...
@flow.math.jit_compile
def crank_nicolson_diffusion_with_obstacles(
    concentration: flow.field.Field,
    obstacle_mask: flow.field.Field,
    diffusion: Numerical,
    dt: Numerical,
    ...
    return (1 - obstacle_mask) * flow.diffuse.implicit(
        concentration, diffusivity=diffusion, dt=dt, order=2
    )
...
class DiffusionSolver2D:
    def __init__(
        ...
        init_value: float = 1,
        diff_coef: float = 1,
        obstacles: List[Geometry] = None,
        grid_res: int or Tuple[int] = None,
    ) -> None:
        # Bounds
        if not (isinstance(x_max, (float, int)) or isinstance(y_max, (float, int))):
            raise ValueError(
                f"Expected values for simulation bounds must be float or int, not {type(x_max), type(y_max)}"
            )
        ...
        if not isinstance(t_max, (float, int)) or t_max <= 0:
            raise ValueError(
                f"Simulation ending time need to be a positive int or float, not {t_max}"
            )
        self.t_max = t_max
        # Obstacles
        if obstacles is None:
            obstacles = []
        assert all(
            [isinstance(obs, Geometry) for obs in obstacles]
        ), "Only instances of Obstacle are allowed."
        self.obstacles = [
            obstacle.to_phiflow() for obstacle in obstacles
        ]  # Creates a phiflow mask array from a list of obstacles
        # Initial distrib
        if not isinstance(source, Geometry):
            raise TypeError(
                f"Initial distribution need be an instance of Rectangle, not {type(source)}"
            )
        if source.center[0] > self._x_max or source.center[1] > self._y_max:
            raise ValueError(
                f"Source center can not be out of bounds ([0,{self._x_max}], [0,{self._y_max}]). Get {source.center}"
            )
        ...
        if diff_coef <= 0:
            raise ValueError(
                f"The diffusion coefficient can not be negative. Get {init_value}"
            )
        ...
        if len(grid_res) != 2:
            raise ValueError(
                f"Too many values to unpack. Grid resolution expect at most 2 values (x_res, y_res). Get {len(grid_res)}"
            )
    ...
    def reset(self):
        ...
        self.concentration = self._init_value * flow.CenteredGrid(
            self._source, bounds=bounds, **self._grid_res
        )
        ...
        self.obstacle_mask = (
            flow.HardGeometryMask(flow.union(self.obstacles)) @ self.concentration
            if self.obstacles
            else None
        )
    ...
    def step(self, concentration) -> flow.CenteredGrid:
        if self.obstacle_mask is None:
            ...
            return crank_nicolson_diffusion_with_obstacles(
                concentration, self.obstacle_mask, self._diff_coef, self.dt
            )

    def start(self, nb_samples: int = 0) -> flow.CenteredGrid:
        if self.time > self.t_max:
            warnings.warn(
                f"The time {self.time} is already greater than the maximum time {self.t_max}. Reset the simulation or set time to 0 in order to start a new simulation."
            )
            return self.concentration

        assert (
            nb_samples >= 0
        ), f"The value for which we want to save the solution must be positive. Get {nb_samples}"
...
def main():
    ...
    env = SnakeClassicEnv(
        width=20, height=20, nb_obstacles=20, pixel=10, max_obs_size=2
    )
    env.reset()
    ...
    logs = flow.math.log(
        flow.math.where(solutions.values < 1e-4, 1e-4, solutions.values)
    )
    grads = flow.math.spatial_gradient(
        logs, padding=flow.extrapolation.ONE * np.log(1e-4)
    )
