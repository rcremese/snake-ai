...
def plot_concentration_map(
    ...
    im = ax.imshow(concentration, origin='lower',cmap=cmap, extent=[0, x_max, 0, y_max])
    # Plot the force field if any
    if force_field is not None:
        gradient = force_field.values.numpy('vector, y, x')
        ax.quiver(np.linspace(0, y_max, int(field.resolution['y'])),
                  np.linspace(0, x_max, int(field.resolution['x'])),
                   gradient[0], gradient[1], angles='xy', scale_units='xy', scale=1)
...
def plot_walkers_with_concentration(
    ...
    if concentration is None:
        ...
        fig, ax, _ = plot_concentration_map(concentration, cmap=cmap_concentration, title=title)
    ...
    if force_field is not None:
        ...
        ax.quiver(positions[:,0], positions[:,1], gradient[:,0], gradient[:,1], angles='xy', scale_units='xy', scale=1)
...
def animate_walk_history(
    ...
    # Create colormap
    fig, ax, scatter = plot_walkers_with_concentration(trajectories.time[0], concentration, target, force_field, cmap_concentration, cmap_walkers, title)
...
if __name__ == "__main__":
    ...
    fig, ax, im = plot_concentration_map(concentration, force_field, title="Concentration map")
    # fig, ax, scatter = plot_walkers_with_concentration(pt_cloud, concentration, force_field=force_field, title="Concentration map + walkers")
