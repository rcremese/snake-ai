...
class BaseDiffusionWrapper(gym.Wrapper, metaclass=ABCMeta):
    def __init__(self, env: SnakeClassicEnv, diffusion_coef: float = 1, seed: int = 0):
        ...
        self.observation_space = gym.spaces.Box(
            low=np.zeros(3), high=np.ones(3), shape=(3,)
        )

        if diffusion_coef < 0:
            raise ValueError(
                f"Diffusion coeffient need to be positive, not {diffusion_coef}"
            )
...
class DeterministicDiffusionWrapper(BaseDiffusionWrapper):
    def _get_diffusion_field(self) -> np.ndarray:
        x, y = np.meshgrid(
            range(self.env.window_size[0]), range(self.env.window_size[0])
        )
        mu = self.env._food.center
        diffusive_field = np.exp(
            -0.5 / self._diffusion_coef * ((x - mu[1]) ** 2 + (y - mu[0]) ** 2)
        )
        assert np.max(diffusive_field) != np.min(
            diffusive_field
        ), "Diffion field is constant"
        return (diffusive_field - np.min(diffusive_field)) / (
            np.max(diffusive_field) - np.min(diffusive_field)
        )
...
class StochasticDiffusionWrapper(BaseDiffusionWrapper):
    def __init__(
        self,
        env: SnakeClassicEnv,
        diffusion_coef: float = 1,
        nb_part: float = 1e6,
        t_max: int = 100,
        seed: int = 0,
    ):
        super().__init__(env, diffusion_coef, seed)
        self._diffusion_process = DiffusionProcess(
            nb_part,
            t_max=t_max,
            window_size=self.env.window_size,
            diff_coef=self._diffusion_coef,
    ...
    def _get_diffusion_field(self) -> np.ndarray:
        ...
        diffusive_field = jsp.signal.convolve(
            concentration_map, conv_window.value, mode="same"
        )
