...
class RelativePositionWrapper(gym.Wrapper):
            ...
            self.observation_space = gym.spaces.Dict(
                {
                    ...
                    "achieved_goal": gym.spaces.Box(
                        low=np.zeros(2),
                        high=np.array(self.env.window_size),
                        shape=(2,),
                        dtype=int,
                    ),  # snake head position
    ...
    def reset(self) -> Tuple[np.array, dict]:
        ...
        observations = self._get_observables_in_local_frame(
            directions, rot_matrix, info["collision_lines"], food_line
        )
    ...
    def step(self, action: int) -> Tuple[np.ndarray, float, bool, bool, dict]:
        ...
        assert isinstance(info, dict) and set(
            ["collision_lines", "snake_direction", "snake_head", "food"]
        ).issubset(info.keys()), info.keys()
        directions = self._get_list_direction(info["snake_direction"])
        rot_matrix = self._get_passing_matrix(info["snake_direction"])
        food_line = Line(info["snake_head"], info["food"])
        observations = self._get_observables_in_local_frame(
    ...
    def _get_observables_in_local_frame(
        self,
        direction_list: List[Direction],
        rot_matrix: np.ndarray,
        collision_lines_dict: Dict[Direction, Line],
        food_line: Line,
    ) -> np.array:
        ...
        for i, direction in enumerate(direction_list):
            ...
            if collision_lines_dict[direction] is None:
                ...
                observations[i, :] = np.matmul(
                    rot_matrix, collision_lines_dict[direction].to_vector()
                )
